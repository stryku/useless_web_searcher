...
namespace usl::parser::content
{
    ...
    {
        const auto body_bounds = find_body_bounds();
        return extract_text(body_bounds);
    }
    ...
    std::string text_content_extractor::extract_text(pos_bounds_t body_bounds) const
    {
        ...
        while(current_pos < body_bounds.second)
        {
            ...
            const auto text_begin = std::next(m_site_content.cbegin(), text_bounds.first);
            const auto text_end = std::next(m_site_content.cbegin(), text_bounds.second);
    ...
    text_content_extractor::pos_bounds_t text_content_extractor::get_next_tag(text_content_extractor::pos_t tag_start) const
    {
    ...
    {
        const auto k_end_script_tag = std::string{ "</script>" };
